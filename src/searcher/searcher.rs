...
pub fn search(mut data: Vec<u8>, data_preparer: Option<Box<dyn DataPreparer>>, mut oracle: Box<dyn Oracle>, checker: Option<Box<dyn Checker>>, start_nonce_size: Option<u64>, step: Option<u64>) -> Vec<u8> {
    ...
    loop {
        ...
        /*
        let h_ptr:Box<dyn Fn(&mut dyn UnitaryBuilder, Register) -> Register> = Box::new(OpBuilder::hadamard);
...
fn diffuse(builder: &mut OpBuilder,mut qubits:Register)->Register{
...
fn toffoli_wrapper(b: &mut dyn UnitaryBuilder, mut rs: Vec<Register>) -> Result<Vec<Register>, CircuitError>{
unimplemented!()
}
